...
let checkRequireFailed = (inputData) => {
  let arrFailed = ['doctorId', 'contentHTML', 'contentMarkdown', 'action', 'selectedPrice', 'selectedPayment',
...
let saveDetailInforDoctor = (inputData) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      let checkObj = checkRequireFailed(inputData)
      if(checkObj.isValid === false){

        resolve({
          errCode: 1,
          errMessage: `không có dữ liệu tham chiếu...: ${checkObj.element}`,
        });
      } else {
        if (inputData.action === "CREATE") {
          ...
          let doctorMarkdown = await db.Markdown.findOne({
            ...
            raw: false, //khi dùng hàm save kiểu sequelize object
          });
          if (doctorMarkdown) {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      if (!data.arrSchedule || !data.doctorId || !data.formatedDate) {
        ...
        // console.log('check diffence ====================================0', toCreate)
        //co su khac biet thi create data
...
module.exports = {
  ...
  checkRequireFailed: checkRequireFailed,
};
